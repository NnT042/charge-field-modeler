...
| 3     | Y-spin    | 4         | Photon |
| 4     | Z-spin    | 8         | Photon |
| 5     | Axial (a) | 16        | Electron |
| 6     | X-spin    | 32        | Electron |
| 7     | Y-spin    | 64        | Electron |
| 8     | Z-spin    | 128       | Electron |
| 9     | Axial (a) | 256       | Baryon |
| 10    | X-spin    | 512       | Baryon |
| 11    | Y-spin    | 1024      | Baryon |
| 12    | Z-spin    | 2048      | Baryon |

**Note:** The pure geometric doubling gives a proton-to-electron radius ratio of 2048/128 = 16. The known ratio from the theory is approximately 1836 (mass ratio) or ~1820 (radius ratio). This discrepancy may be resolved by energy/mass scaling factors that modify the effective radius beyond pure geometric doubling. This is a known open question to investigate during development. The geometric amplitudes above define the spin path; the *effective* radius perceived by external particles may differ.
...
struct SpinLevel {
    ...
    tier: Tier,             // Photon, Electron, Baryon
...
struct FocusParticle {
    spin_stack: Vec<SpinLevel>,     // active spin levels (1 to 12)
...
### M1: Spinning Sphere
- [ ] Godot project scaffold with Rust GDExtension building and loading
- [ ] Single sphere rendered with PBR shading
- [ ] Axial spin slider (±c range) with real-time rotation
- [ ] Basic orbit camera
...
- [ ] Tier transitions (photon → electron → baryon)
- [ ] Auto-zoom on level addition
- [ ] GPU-accelerated position computation (compute shader for path trace at high level counts)
